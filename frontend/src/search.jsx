...
export default function MatrimonySearch() {
  ...
  const Sel = ({ label, value, onChange, children }) => (
    <div>
      <label style={{ fontSize:14, fontWeight:700, color:'#888', textTransform:'uppercase', letterSpacing:0.5, marginBottom:3, display:'block' }}>{label}</label>
      <select value={value} onChange={e => onChange(e.target.value)}
        style={{ width:'100%', padding:'9px 10px', border:'1.5px solid #e0e0e0', borderRadius:8, fontSize:17, background:'#fff', color:'#333', outline:'none', appearance:'none', cursor:'pointer',
  ...
  const Inp = ({ label, ...props }) => (
    <div>
      <label style={{ fontSize:14, fontWeight:700, color:'#888', textTransform:'uppercase', letterSpacing:0.5, marginBottom:3, display:'block' }}>{label}</label>
      <input {...props} style={{ width:'100%', padding:'9px 10px', border:'1.5px solid #e0e0e0', borderRadius:8, fontSize:17, background:'#fff', color:'#333', outline:'none', boxSizing:'border-box', ...(props.style||{}) }} />
    </div>
  );
    ...
    <div style={{ background:'#F4FAF8', minHeight:'100vh', paddingBottom:70 }}>
      ...
      <div style={{ background:'#fff', padding:'10px 16px', borderBottom:'1px solid #f0f0f0', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:56, zIndex:50 }}>
        <div style={{ whiteSpace:'nowrap' }}>
          ...
          <span style={{ fontSize:16, color:'#999', marginLeft:4 }}>· {totalProfiles} found</span>
        </div>
        <div style={{ display:'flex', gap:6 }}>
          <button onClick={() => setShowFilters(!showFilters)}
            style={{ padding:'6px 12px', borderRadius:20, border:'1.5px solid #e0e0e0', background: showFilters ? '#0D7B6A' : '#fff', color: showFilters ? '#fff' : '#666', fontSize:15, fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', gap:4 }}>
            ...
            Filters {activeFilterCount > 0 && <span style={{ background:'#6B3FA0', color:'#fff', borderRadius:10, padding:'0 5px', fontSize:13, fontWeight:700 }}>{activeFilterCount}</span>}
          </button>
          <select value={applied.sortId} onChange={e => { const v = e.target.value; const nf={...filters,sortId:v}; const na={...applied,sortId:v}; setFilters(nf); setApplied(na); setCurrentPage(1); fetchProfiles(na,1); }}
            style={{ padding:'6px 10px', borderRadius:20, border:'1.5px solid #e0e0e0', fontSize:15, fontWeight:600, color:'#666', cursor:'pointer', background:'#fff', outline:'none' }}>
      ...
      {showFilters && (
        <div style={{ background:'#fff', padding:'14px 16px', borderBottom:'1px solid #f0f0f0' }}>
          ...
          <button onClick={() => setShowAdvanced(!showAdvanced)}
            style={{ marginTop:10, background:'none', border:'none', color:'#0D7B6A', fontSize:16, fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', gap:4, padding:0 }}>
          ...
          <div style={{ display:'flex', gap:8, marginTop:12 }}>
            <button onClick={doSearch}
              style={{ flex:1, padding:'10px', background:'linear-gradient(135deg,#0D7B6A,#6B3FA0)', color:'#fff', border:'none', borderRadius:8, fontSize:18, fontWeight:700, cursor:'pointer' }}>
              Search
            </button>
            <button onClick={doReset}
              style={{ padding:'10px 20px', background:'#fff', color:'#0D7B6A', border:'1.5px solid #0D7B6A', borderRadius:8, fontSize:17, fontWeight:600, cursor:'pointer' }}>
              Reset
            </button>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign:'center', padding:60, color:'#bbb' }}>
          ...
          <div style={{ fontSize:16, color:'#ccc', marginTop:4 }}>Try adjusting your filters</div>
        </div>
      ) : (
          ...
          {paged.map(p => {
            ...
            return (
              <div key={p.id} style={{ background:'#fff', borderRadius:14, overflow:'hidden', boxShadow:'0 2px 10px rgba(0,0,0,0.06)', border:'1px solid #efeaf3', cursor:'pointer' }}
                ...
                {/* Photo banner */}
                <div style={{ position:'relative', width:'100%', aspectRatio:'4 / 3', background:'#F4FAF8' }}>
                  {p.photo ? (
                    <picture>
                  ...
                  <span style={{ position:'absolute', top:10, right:10, background:'rgba(13,123,106,0.85)', color:'#fff', fontSize:15, fontWeight:700, padding:'3px 10px', borderRadius:14, backdropFilter:'blur(4px)' }}>1 / 1</span>
                </div>
                ...
                <div style={{ padding:'10px 14px 6px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:8 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', minWidth:0 }}>
                    <span style={{ fontSize:16, color:'#0D7B6A', fontWeight:700, letterSpacing:0.3 }}>{p.regId}</span>
                    {p.religion && <span style={{ fontSize:15, fontWeight:600, padding:'2px 9px', borderRadius:12, background:'#ecfdf5', color:'#047857', border:'1px solid #a7f3d0' }}>{p.religion}</span>}
                  </div>
                  <button onClick={e => { e.stopPropagation(); goDetail(); }}
                    style={{ display:'flex', alignItems:'center', gap:5, background:'linear-gradient(135deg,#0D7B6A,#6B3FA0)', color:'#fff', border:'none', borderRadius:18, padding:'6px 12px', fontSize:15, fontWeight:700, cursor:'pointer', boxShadow:'0 1px 4px rgba(13,123,106,0.3)', whiteSpace:'nowrap' }}>
                ...
                <div style={{ padding:'2px 14px 4px' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                    <span style={{ color:'#0D7B6A', fontSize:18 }}>📍</span>
                    <span style={{ fontSize:20, fontWeight:700, color:'#1a1a2e', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p.name}</span>
                  </div>
                </div>

                {/* Tag pills */}
                <div style={{ padding:'2px 14px 6px', display:'flex', gap:6, flexWrap:'wrap' }}>
                  {p.caste && <span style={{ fontSize:15, fontWeight:600, padding:'3px 9px', borderRadius:6, background:'#f3f4f6', color:'#374151' }}>Caste: {p.caste}</span>}
                  {p.marital && <span style={{ fontSize:15, fontWeight:600, padding:'3px 9px', borderRadius:6, background:'#f3f4f6', color:'#374151' }}>Status: {p.marital}</span>}
                </div>

                {/* Info row */}
                <div style={{ padding:'4px 14px 10px', display:'flex', gap:14, flexWrap:'wrap', fontSize:16, color:'#4b5563' }}>
                ...
                <div style={{ background:'linear-gradient(135deg,#E8F5F2,#F4FAF8)', borderTop:'1px solid #C8EDE6', borderBottom:'1px solid #C8EDE6', display:'flex' }}>
                  <div style={{ flex:1, padding:'10px 14px', borderLeft:'3px solid #0D7B6A' }}>
                    <div style={{ fontSize:13, fontWeight:700, color:'#9ca3af', letterSpacing:0.5, textTransform:'uppercase' }}>Education</div>
                    <div style={{ fontSize:17, fontWeight:700, color:'#0D7B6A', marginTop:2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p.qualification || '—'}</div>
                  </div>
                  <div style={{ flex:1, padding:'10px 14px', textAlign:'right' }}>
                    <div style={{ fontSize:13, fontWeight:700, color:'#9ca3af', letterSpacing:0.5, textTransform:'uppercase' }}>Occupation</div>
                    <div style={{ fontSize:17, fontWeight:700, color:'#1a1a2e', marginTop:2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p.job || '—'}</div>
                  </div>
                </div>

                {/* Footer: location + view hint */}
                <div style={{ padding:'8px 14px', display:'flex', alignItems:'center', justifyContent:'space-between', fontSize:15, color:'#6b7280', background:'#fafafa' }}>
                  ...
                  <span style={{ display:'flex', alignItems:'center', gap:4, color:'#0D7B6A', fontWeight:600, whiteSpace:'nowrap' }}>
                    📞 View Contact
                  </span>
                </div>
              </div>
          ...
          {totalPages > 1 && (
            <div style={{ display:'flex', justifyContent:'center', alignItems:'center', gap:12, padding:'12px 0' }}>
              <button disabled={currentPage===1 || loading} onClick={() => goToPage(currentPage - 1)}
                style={{ padding:'8px 16px', borderRadius:8, border:'1.5px solid #e0e0e0', background:'#fff', color: currentPage===1 ? '#ccc' : '#0D7B6A', fontSize:17, fontWeight:600, cursor: currentPage===1 ? 'not-allowed' : 'pointer' }}>
                Prev
              </button>
              <span style={{ fontSize:17, color:'#999' }}>Page <strong>{currentPage}</strong> / <strong>{totalPages}</strong></span>
              <button disabled={currentPage===totalPages || loading} onClick={() => goToPage(currentPage + 1)}
                style={{ padding:'8px 16px', borderRadius:8, border:'1.5px solid #e0e0e0', background: currentPage===totalPages ? '#fff' : '#0D7B6A', color: currentPage===totalPages ? '#ccc' : '#fff', fontSize:17, fontWeight:600, cursor: currentPage===totalPages ? 'not-allowed' : 'pointer' }}>
                Next
              </button>
            </div>
          )}
